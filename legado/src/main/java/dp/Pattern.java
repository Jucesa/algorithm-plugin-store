...
//        }
     ...
     * Lógica: Se para t0do exemplo negativo, sempre que o vrN=false
    ...
    /**
     * Retorna se indivíduo this cobre os mesmos exemplos positivos
...
//    
    ...
    public String toString2() {
        ...
        int indice = 0;
        while(iterator.hasNext()){
            itensArray[indice++] = iterator.next();
        }           
        Arrays.sort(itensArray);
        ...
        int j = 0;
        for(; j < itensArray.length-1; j++){
            str.append(D.itemAtributoStr[itensArray[j]]).append(" = ").append(D.itemValorStr[itensArray[j]]);
            str.append(",");
        }
        if(!itens.isEmpty()){
            str.append(D.itemAtributoStr[itensArray[j]]).append(" = ").append(D.itemValorStr[itensArray[j]]);
        ...
        str.append("} -> (");
        str.append(D.valorAlvo).append(":").append(this.TP).append(", not").append(D.valorAlvo).append(":").append(this.FP).append(") ");
        str.append("(conf=");
    ...
    public String toString3() {
        ...
        str.append("\n"); 
        if(this.similares != null){
            for (Pattern similares : this.similares) {
                str.append("\t");
                str.append(similares.toString3());
            }
        }
          return str.toString(); 
    }
    ...
    public String toString(String[] metricas, boolean imprimirCoberturaDp, boolean imprimirCoberturaDn, boolean imprimirSimilares) {
        ...
        str.append(itensArray[j]);
        str.append("}->").append(D.valorAlvo);
        str.append(" (").append(D.valorAlvo).append("=").append(this.TP).append(", outros=").append(this.FP).append(")");
            ...
            for(int i = 0; i < metricas.length; i++){
                ...
                switch(metrica){
                    case Const.METRICA_QUALIDADE:
                        str.append("Qualidade=").append(df4.format(this.qualidade));
                        break;
                    case Const.METRICA_WRACC:
                        str.append("WRAcc=").append(df4.format(Avaliador.WRAcc(this.TP, this.FP)));
                        break;
                    case Const.METRICA_Qg:
                        str.append("Qg=").append(df2.format(Avaliador.Qg(this.TP, this.FP)));
                        break;
                    case Const.METRICA_CHI_QUAD:
                        str.append("Chi2=").append(df2.format(Avaliador.chi_quad(this.TP, this.FP)));
                        break;    
                    case Const.METRICA_P_VALUE:
                        str.append("pValue=").append(df4.format(Avaliador.p_value(this.TP, this.FP)));
                        break;
                    case Const.METRICA_LIFT:
                        str.append("Lift=").append(df2.format(Avaliador.lift(this.TP, this.FP)));
                        break;
                    case Const.METRICA_DIFF_SUP:
                        str.append("DiffSup=").append(df4.format(Avaliador.DiffSup(this.TP, this.FP)));
                        break;
                    case Const.METRICA_SIZE:
                        str.append("Size=").append(df2.format(this.itens.size()));
                        break;
                    case Const.METRICA_COV:
                        str.append("Cov=").append(df4.format(Avaliador.cov(this.TP, this.FP)));
                        break;
                    case Const.METRICA_CONF:
                        str.append("Conf=").append(df4.format(Avaliador.conf(this.TP, this.FP)));
                        break;
                    case Const.METRICA_SUPP_POSITIVO:
                        str.append("Sup(+)=").append(df4.format(Avaliador.suppPositivo(this.TP)));
                        break;
                    case Const.METRICA_SUPP_NEGATIVO:
                        str.append("Sup(-)=").append(df4.format(Avaliador.suppNegativo(this.FP)));
                        break;                    
                }
        ...
        if(imprimirSimilares && this.similares != null){            
            ...
            for(int i = 0; i < this.similares.length; i++){
                str.append("   S(").append(i).append("):");
                str.append(this.similares[i].toString(metricas, imprimirCoberturaDp, imprimirCoberturaDn, !imprimirSimilares));
                str.append(" ||Similarity:").append(df4.format(Avaliador.similaridade(this, this.similares[i], Pattern.medidaSimilaridade))).append("||");
                str.append("\n");
            }
        }
        return str.toString(); 
